...
def add_eval_options(parser, main=False, comparison=False, pretrain=False, compare_replay=False, **kwargs):
    ...
    eval_params.add_argument('--results-dict', action='store_true', help="output dict with results after each context")
    if not comparison:
    ...
    if (not comparison) and (not pretrain):
...
def add_train_options(parser, main=False, pretrain=False, compare_replay=False, **kwargs):
...
def add_cl_options(parser, main=False, compare_all=False, compare_replay=False, compare_hyper=False,
    ...
    if main:
        replay_choices = ['none', 'all', 'generative', 'current', 'buffer']
    ...
    if (not compare_replay):
        ...
        if main:
            replay_params.add_argument('--dg-gates', action='store_true', help="use class-specific gates in decoder")
        if not compare_hyper:

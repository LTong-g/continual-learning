...
def show_samples(model, config, pdf=None, visdom=None, size=32, pdf_title="Generated images", visdom_title="Samples"):
    '''Plot samples from a generative model in [pdf].'''
...
def show_reconstruction(model, dataset, config, pdf=None, visdom=None, size=32, context=None):
    '''Plot reconstructed examples by an auto-encoder [model] on [dataset], in [pdf].'''
    ...
    comparison = torch.cat(
        [data.view(-1, config['channels'], config['size'], config['size'])[:int(size/2)],
         recon_batch.view(-1, config['channels'], config['size'], config['size'])[:int(size/2)]]
    )

    nrow = int(np.ceil(np.sqrt(size)))
    if pdf is not None:
        visual_plt.plot_images_from_tensor(comparison.cpu(), pdf, nrow=nrow, title="Reconstruction")
